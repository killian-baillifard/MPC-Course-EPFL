...
class TubeMPCControl_base(MPCControl_base):
	...
	def __init__(
		...
		for k in range(self.N):
			cost += cp.quad_form(self.dz_var[:, k], Q)
			cost += cp.quad_form(self.dv_var[:, k], R)

		# Define delta dynamics
		
		dynamics = [
			self.dz_var[:, 1:] == self.A @ self.dz_var[:, :-1] + self.B @ self.dv_var
		]
	...
	def get_u(
		...
		dz0 = self.dz_var.value[:, 0]
		dv0 = self.dv_var.value[:, 0]
		dx0 = x0.reshape(-1) - self.xs.reshape(-1)
		du0 = (self.K @ (dx0 - dz0)).reshape(-1) + dv0
		u0 = (self.us.reshape(-1) + du0).reshape(-1)

		# Return open loop prediction

		x_traj = self.dz_var.value + self.xs.reshape(-1, 1)
		u_traj = self.dv_var.value + self.us.reshape(-1, 1)
