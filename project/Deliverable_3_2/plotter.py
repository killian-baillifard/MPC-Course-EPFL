...
def plot_traj(name: str, x: np.ndarray, u: np.ndarray, t: np.ndarray) -> None:
    ...
    plt.subplot(221)
    plt.plot(t[:-1], x[9, :-1], color=X_COLOR, linestyle=(0, (1, 3)), label=r'$p_x$')
    plt.plot(t[:-1], x[10, :-1], color=Y_COLOR, linestyle=(2, (1, 3)), label=r'$p_y$')
    plt.plot(t[:-1], x[11, :-1], color=Z_COLOR, label=r'$p_z$')
    plt.ylabel(r'$p_x, \quad p_y, \quad p_z \quad (m)$')
    plt.grid()
    ...
    twin.plot(t[:-1], np.rad2deg(x[0, :-1]), color='xkcd:magenta', label=r'$\omega_\alpha$')
    twin.plot(t[:-1], np.rad2deg(x[1, :-1]), color='xkcd:cyan', label=r'$\omega_\beta$')
    twin.plot(t[:-1], np.rad2deg(x[2, :-1]), color='xkcd:purple', label=r'$\omega_\gamma$')
